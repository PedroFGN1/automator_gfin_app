...
async function executarRestituicaoDeducaoReceita(configPerfil, caminhoExcel, diretorioSaida, enviarLog, controle) {
    ...
    logTotal(`🚀 Inicializando Bot: ${configPerfil.nome}...`);
    let browser = null;
    ...
    try {
        ...
        logTotal('🔐 Acessando o portal para Login...');
        ...
        for (let i = 0; i < dados.length; i++) {
            ...
            try {
                ...
                await pptUtils.selecionarOpcaoPorTexto(
                    contexto, 
                    'txtFinalidade', 
                    configPerfil.configuracoes_fixas.texto_selecao_inicial || 'Dedução de Receita com Restituição de Recursos'
                );
                await navUtils.delay(500); // Espera carregamento
                ...
                const orgaoCodigo = configPerfil.configuracoes_fixas.orgao_codigo || '9995';
                await pptUtils.preencherTexto(contexto, 'txtOrgao', orgaoCodigo);
                ...
                let dataRestituicao = linha[configPerfil.mapeamento_colunas.DATA];
                let dataObj = null;
                if (dataRestituicao) {
                ...
                await contexto.evaluate((nome, doc, id) => {
                    ...
                    if (iNome) { 
                        iNome.removeAttribute('readonly'); 
                        iNome.removeAttribute('onchange'); 
                        iNome.value = nome;
                    }
                ...
                // ------- Débito (Receita Dinâmica ou Fixa)
                const receitaDebito = linha[configPerfil.mapeamento_colunas.RECEITA_DEBITO] || 
                                      configPerfil.configuracoes_fixas.receita_debito || 
                                      configPerfil.configuracoes_fixas.receita_debito_padrao;

                if (!receitaDebito) {
                    throw new Error('Código de Receita Débito não informado na planilha nem no perfil.');
                }

                await pptUtils.preencherTexto(contexto, 'txtCodigoReceitaDebito', String(receitaDebito).trim());
                ...
                await pptUtils.selecionarOpcaoPorTexto(
                    contexto, 
                    'txtTipoContaDebito', 
                    configPerfil.configuracoes_fixas.tipo_debito_texto || 'CUTE'
                );
                await navUtils.delay(500);

                // Conta Débito (Injeção)
                const debito_names = {
                ...
                // ------- Crédito
                const bancoCredito = linha[configPerfil.mapeamento_colunas.BANCO_CREDITO];
                const agenciaCredito = linha[configPerfil.mapeamento_colunas.AGENCIA_CREDITO];
                const contaCredito = linha[configPerfil.mapeamento_colunas.CONTA_CREDITO];
                const possuiDadosCredito = [bancoCredito, agenciaCredito, contaCredito].some(valor =>
                    valor !== null && valor !== undefined && String(valor).trim() !== ''
                );

                if (possuiDadosCredito) {
                    await pptUtils.selecionarOpcaoPorTexto(
                        contexto, 
                        'txtTipoContaCredito', 
                        configPerfil.configuracoes_fixas.tipo_credito_texto || 'Movimento'
                    );
                    await navUtils.delay(300);

                    await pptUtils.preencherTexto(contexto, 'txtBancoCredito', bancoCredito);
                    await pptUtils.preencherTexto(contexto, 'txtAgenciaCredito', agenciaCredito);
                    await pptUtils.preencherTexto(contexto, 'txtContaCredito', contaCredito);
                }

                // ------- Lista de Credores (SEMPRE DESMARCADO)
                await contexto.evaluate(() => {
                    const chkCredores = document.querySelector('input[name="txtListaCredores"]');
                    if (chkCredores) {
                        chkCredores.checked = false;
                        chkCredores.value = 'N';
                    }
                });
                await navUtils.delay(200);

                // ------- Histórico
                const textoHistorico = linha[configPerfil.mapeamento_colunas.HISTORICO] || '';
                await pptUtils.preencherTexto(contexto, 'txtHistorico', textoHistorico);
                await navUtils.delay(300);

                // ------- Opções Finais (Enviar para Banco & Rascunho)
                const enviarBanco = configPerfil.configuracoes_fixas.enviar_para_banco || 'S';
                await pptUtils.marcarOpcao(contexto, 'txtEnviarBanco', enviarBanco);
                await navUtils.delay(300);

                const isRascunho = configPerfil.configuracoes_fixas.rascunho || 'N';
                await pptUtils.marcarOpcao(contexto, 'txtIsRascunho', isRascunho);
                await navUtils.delay(300);

                // ------- D. Submissão (Incluir)
                let mensagemErroAlert = null;

                // Prepara ouvinte de Alert/Dialog
                const listenerDialog = async d => {
                    mensagemErroAlert = d.message();
                ...
                if (botaoIncluir) {
                    ...
                    try {
                        ...
                        // Se der timeout, alert pode ter aparecido
                    }
                }

                // Remove ouvinte de dialog
                page.off('dialog', listenerDialog);

                // Verificação de erro da inclusão
                ...
                if (botaoConfirmarSim) {
                    ...
                    await page.screenshot({ 
                        path: path.join(diretorios.evidencias, `${idProcesso}_CONFERÊNCIA.png`), 
                        fullPage: true 
                    });
                    ...
                    await page.screenshot({ 
                        path: path.join(diretorios.evidencias, `${idProcesso}_ERRO_INCERTO.png`), 
                        fullPage: true 
                    });
                }

                // Captura do Número da OP
                let textoCapturado = '';
                let numeroOP = 'Não gerado';
...
module.exports = { executarRestituicaoDeducaoReceita };
